...
```{r}
netzwerk <-
  graph_from_literal(Susi -+ Max, Susi -+ Ali, Susi -+ Til,
                     Eda -+ Lena, Ella, Lena -+ Eda,
                     Max -+ Ali, Max -+ Susi, Ali -+ Max,
                     Ali -+ Susi, Til -+ Susi)
```
...
```{r}
par(mar = c(0, 0, 0, 0))
# Ränder (margins) der Grafik auf null setzten
...
```{r}
# Variable des Beispieldatensatzes aufrufen und anzeigen
beispieldatensatz$IV
...
```{r}
daten <-
  read.csv2("https://figshare.com/ndownloader/files/31108225")
```
...
Welche Dimension einer Sozialbeziehung soll fokussiert werden (z.B. Beliebtheit, Teamarbeit, usw.)? Soll ein konkreter zeitlicher Rahmen gesetzt werden (z.B. gestern, heute, morgen, aktuelles Schuljahr, usw.)? Soll ein konkretes Setting abgefragt werden (z.B. in der Pause, im Kunstunterricht, auf dem Ausflug)?
